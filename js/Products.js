...
$(document).ready(() => {
    $.get("https://5fc1a1c9cb4d020016fe6b07.mockapi.io/api/v1/products", (productPageData) => {
         class Product {
            ...
            printProduct() {
                return (`<tr>
                            <td class="column1">${this.pid}</td>
                            <td class="column2">${this.Name}</td>
                            <td class="column3">${this.Brand}</td>
                            <td class="column4">${this.expiryData}</td>
                            <td class="column5">${this.UnitPrice}</td>
                            <td class="column6">${this.stock}</td>
                        </tr>`
                        )
            }
        }
    ...
    if (allContent.length > 0) {
        ...
        filteredContent = allContent.filter((Product) => {
             if (!expired && !lowStock) return true;
             else if(expired && !lowStock) return new Date(TodayDate) > new Date(Product.expiryDate);
             else if(lowStock && !expired) return (Product.stock < 100);
             else if(expired && lowStock) return ((new Date(TodayDate) > new Date(Product.expiryDate)) && (Product.stock < 100));
